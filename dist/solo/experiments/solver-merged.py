"""
solver-merged.py — local experiment, not a submitted artifact.
...
at the end of this header.
  ...
  Adds the parsimagma refuter stage: linear and affine magmas
  over Z/m, ordered by a greedy set cover of the ETP's false
  implications, then (Z/p)^k cocycle extensions.

Measured on a local 200-problem sample set, NOT the graded set --
the organizer evaluates on a private set that reuses no publicly
available problem. See ../../docs/merge-experiment.md for what the
numbers mean and, more to the point, what they do not.
...
def pm_search_extension(hyp, concl, deadline, max_carrier=10):
